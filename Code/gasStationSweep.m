...
%             time_left = process_time*left;
            waitbar(round(count/total_iter,3),f,"Tank Size: ("+t+"/"+nt+")."+newline+"Number pumps: ("+n+"/"+np+")."+newline+"Iteration: ("+i+"/"+iters+")")
            if isnan(temps(i))
                temps(i:end) = NaN;
                count = count + np-i;
                break
